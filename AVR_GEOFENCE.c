...
*/
PROGMEM static const float polyARG[] = {
	-68.0967,	-57.0310,
	-47.0506,	-35.9905,
	-57.4657,	-30.2155,
	-60.1426,	-22.0487,
...
PROGMEM static const float polyAUS[] = {
	105.6095,	-18.2615,
	105.9172,	-36.5192,
	152.0158,	-49.0600,
	164.5842,	-26.2647,
...
PROGMEM static const float polyBRA[] = {
	-47.2264,	5.5786,
	-64.1893,	5.7535,
	-75.5272,	-8.4877,
	-62.3126,	-14.4031,
...
PROGMEM static const float polyCHN[] = {
	87.5641,	49.8807,
	65.8377,	45.1225,
	66.1627,	38.4280,
	79.9615,	29.9362,
...
PROGMEM static const float polyJPN[] = {
	124.5683,	21.2113,
	132.4496,	21.2116,
	146.3613,	30.8631,
	159.7969,	45.4214,
...
PROGMEM static const float polyNZ[] = {
	155.2130,	-46.8897,
	176.8341,	-54.2904,
	179.9922,	-50.4242,
	179.9922,	-30.4242,
...
PROGMEM static const float polyUK[] = {
	-5.7579,	50.0109,
	1.2678,		50.8958,
	1.7842,		52.5493,
	-1.4681,	55.3289,
...
int32_t pointInPolygonF(int32_t polyCorners, const float * polygon, float latitude, float longitude)
{
	...
	{
		if((pgm_read_float(&polygon[i + 1]) < latitude && pgm_read_float(&polygon[j + 1]) >= latitude
		|| pgm_read_float(&polygon[j + 1]) < latitude && pgm_read_float(&polygon[i + 1]) >= latitude)
		&& (pgm_read_float(&polygon[i]) <= longitude || pgm_read_float(&polygon[j]) <= longitude))
		{
			oddNodes ^= (pgm_read_float(&polygon[i]) + (latitude - pgm_read_float(&polygon[i + 1]))
			/ (pgm_read_float(&polygon[j + 1]) - pgm_read_float(&polygon[i + 1]))
			* (pgm_read_float(&polygon[j]) - pgm_read_float(&polygon[i])) < longitude);
		}
